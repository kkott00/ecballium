...
;
  ...
  var DELAY, Ecballium, EcballiumMouse, URL;
  ...
  */
  ...
  Ecballium = (function() {
    ...
    Ecballium.prototype.DELAY = DELAY;
    ...
    function Ecballium() {
      ...
      load("" + URL + "config.js").done(function() {
        var i, _i, _len, _ref, _results;
        _ref = ecb_config.modules;
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          i = _ref[_i];
          _results.push(load("" + URL + i));
        }
        return _results;
      });
      this.mouse = new EcballiumMouse();
      this.overlay = $('<div>');
      this.overlay.css({
        'z-index': 1000,
    ...
    Ecballium.prototype.state_machine = function(state) {
      var _this = this;
      if (state === 'init') {
        return wait(200).done(function() {
          var c;
          c = $.cookie('ecballium');
          if (c && ecb_config.policy === 'continue') {
            _this.persist = JSON.parse(c);
            return _this.next('find_next_step');
          } else {
            _this.save_persist({
              id: Math.round(Math.random() * 1e10)
            });
            return _this.next('get_cur_step');
          }
        });
        ...
        this.post('all tests done', 'all tests done');
        return $.cookie('ecballium', null);
      } else {
        throw "unknown state " + state;
      }
    };

    Ecballium.prototype.save_persist = function(obj) {
      if (obj) {
        $.extend(this.persist, obj);
      }
      return $.cookie('ecballium', JSON.stringify(this.persist));
    };
    ...
    Ecballium.prototype.get_file = function(file) {
      ...
      if (!(file in this.files)) {
        $.get("" + URL + file, null, null, 'text').done(function(data) {
    ...
    Ecballium.prototype.find_next_step = function() {
      var _this = this;
      return this.get_file(ecb_config.features[this.loc.file]).done(function() {
        var file, scn;
        scn = _this.loc2scn();
        _this.loc.step += 1;
        if (_this.loc.step >= scn.steps.length) {
          ...
          if (_this.loc.scn >= file.scenarios.length) {
            _this.loc.scn = 0;
            _this.loc.file += 1;
            if (_this.loc.file >= ecb_config.features.length) {
              _this.next('all_done');
              return;
            }
          }
    ...
    Ecballium.prototype.loc2scn = function() {
      return this.files[ecb_config.features[this.loc.file]].scenarios[this.loc.scn];
    };

    Ecballium.prototype.loc2file = function() {
      return this.files[ecb_config.features[this.loc.file]];
    };

    Ecballium.prototype.run_step = function() {
      var d, i, m, step, _i, _len, _ref,
        _this = this;
      step = this.loc2step();
      _ref = this.handlers;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      try {
        d = i[1].apply(this, m.slice(1));
        this.post('success', 'success');
      } catch (e) {
        d = this.show_message(this.mouse.x, this.mouse.y, e.stack, 'rgba(255,0,0,0.5)');
        this.last_exception = e;
        this.post('test failed', e.stack);
        if (this.skipScnOnError) {
          this.loc.step = 1e10;
        }
      }
      if (d && ('promise' in d)) {
    ...
    Ecballium.prototype.post = function(status, msg) {
      ...
      return console.log('===', status, ' = ', data.step, data);
    };

    Ecballium.prototype.assert = function(cond, msg) {
      if (msg == null) {
        msg = '';
      }
      this.skipScnOnError = false;
      if (!cond) {
        throw Error(msg);
      }
    };

    Ecballium.prototype.fail = function(cond, msg) {
      if (msg == null) {
        msg = '';
      }
      this.skipScnOnError = true;
      if (!cond) {
        throw Error(msg);
      }
    };

    Ecballium.prototype.dump_css = function(obj, v) {
      var i, out;
      out = {};
      for (i in v) {
        out[i] = obj.css(i);
    ...
    Ecballium.prototype.show_message = function(x, y, msg, color) {
      var caption, old,
        _this = this;
      if (color == null) {
      ...
      old = this.dump_css(this.overlay, {
      ...
      return wait(DELAY).done(function() {
        _this.overlay.hide();
  ...
  EcballiumMouse = (function() {

    EcballiumMouse.prototype.x = 300;

    EcballiumMouse.prototype.y = 300;

    function EcballiumMouse() {
      this.el = $('<div>');
      this.el.css({
        position: 'absolute',
        top: this.y,
    ...
    EcballiumMouse.prototype.moveto = function(x, y) {
      var d,
        _this = this;
      d = $.Deferred();
      this.x = x - 25;
    ...
    EcballiumMouse.prototype.say = function(say) {
      var pause,
        _this = this;
      this.text.html(say);
      this.el.append(this.text);
      pause = DELAY + say.length;
      return wait(pause).done(function() {
        return _this.text.detach();
      });
    };
